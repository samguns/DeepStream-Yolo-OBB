...
static void
clear_mask_params(NvDsObjectMeta *obj_meta)
{
...
static void
draw_obb_from_meta(NvDsBatchMeta *batch_meta, NvDsFrameMeta *frame_meta, NvDsObjectMeta *obj_meta)
{
  NvDsDisplayMeta *display_meta = NULL;

  guint num_points = obj_meta->mask_params.size / (sizeof(float) * OBB_POINT_SIZE);
  if (!obj_meta->mask_params.data) {
    return;
  }

  if (num_points < OBB_POINT_COUNT || obj_meta->mask_params.width == 0 || obj_meta->mask_params.height == 0) {
    clear_mask_params(obj_meta);
    return;
  }
  ...
  gfloat points[OBB_POINT_COUNT][2];
  for (guint i = 0; i < OBB_POINT_COUNT; ++i) {
    points[i][0] = (obj_meta->mask_params.data[i * OBB_POINT_SIZE + 0] - pad_x) / gain;
    points[i][1] = (obj_meta->mask_params.data[i * OBB_POINT_SIZE + 1] - pad_y) / gain;
  }
  ...
  for (guint i = 0; i < OBB_POINT_COUNT; ++i) {
    ...
    guint next = (i + 1) % OBB_POINT_COUNT;

    NvOSD_LineParams *line_params = &display_meta->line_params[display_meta->num_lines];
    line_params->x1 = (guint) MIN(STREAMMUX_WIDTH - 1, MAX(0, points[i][0]));
    line_params->y1 = (guint) MIN(STREAMMUX_HEIGHT - 1, MAX(0, points[i][1]));
    line_params->x2 = (guint) MIN(STREAMMUX_WIDTH - 1, MAX(0, points[next][0]));
    line_params->y2 = (guint) MIN(STREAMMUX_HEIGHT - 1, MAX(0, points[next][1]));
...
nvosd_sink_pad_buffer_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data)
{
  ...
  for (l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next) {
    ...
    for (l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      draw_obb_from_meta(batch_meta, frame_meta, obj_meta);
