...
class DeepStreamOutput(nn.Module):
    ...
    def forward(self, x):
        ...
        class_scores = x[:, :, 4:-1]
        angle = x[:, :, -1:]
        scores, classes = torch.max(class_scores, dim=-1, keepdim=True)
        return torch.cat([boxes, scores, classes.to(boxes.dtype), angle], dim=-1)
...
def main(args):
    ...
    print("Opening YOLO26-OBB model")
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description="DeepStream YOLO26-OBB conversion")
    parser.add_argument("-w", "--weights", required=True, type=str, help="Input weights (.pt) file path (required)")
    parser.add_argument("-s", "--size", nargs="+", type=int, default=[1024], help="Inference size [H,W] (default [1024])")
