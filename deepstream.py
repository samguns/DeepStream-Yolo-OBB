...
def clear_mask_params(obj_meta):
    if getattr(obj_meta.mask_params, "data", None):
        try:
            pyds.free_buffer(obj_meta.mask_params.data)
        except (AttributeError, TypeError):
            pass

    try:
        obj_meta.mask_params.data = None
    except (AttributeError, TypeError):
        pass
...
def draw_obb_from_meta(batch_meta, frame_meta, obj_meta):
    display_meta = None

    if not getattr(obj_meta.mask_params, "data", None):
        return

    num_points = int(obj_meta.mask_params.size / (sizeof(c_float) * OBB_POINT_SIZE))
    if num_points < OBB_POINT_COUNT or obj_meta.mask_params.width == 0 or obj_meta.mask_params.height == 0:
    ...
    points = []
    for i in range(OBB_POINT_COUNT):
        x = (data[i * OBB_POINT_SIZE + 0] - pad_x) / gain
        y = (data[i * OBB_POINT_SIZE + 1] - pad_y) / gain
        points.append((x, y))
    ...
    for i in range(OBB_POINT_COUNT):
        ...
        next_index = (i + 1) % OBB_POINT_COUNT
        x1, y1 = points[i]
...
def main():
    ...
    if not nvstreammux.link(nvinfer) or not nvinfer.link(nvvidconv) or not nvvidconv.link(capsfilter) or \
            not capsfilter.link(nvosd) or not nvosd.link(nvsink):
        sys.stderr.write("ERROR - Failed to link pipeline elements\n")
        return -1
