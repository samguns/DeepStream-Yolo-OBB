...
Copy the `export_yolo11_obb.py` file from `DeepStream-Yolo-OBB/utils` directory to the `ultralytics` folder.

#### 3. Download the model

Download the `pt` file from [YOLO11](https://github.com/ultralytics/assets/releases/) releases (example for YOLO11s-OBB)

```
wget https://github.com/ultralytics/assets/releases/download/v8.4.0/yolo11s-obb.pt
```
...
Generate the ONNX model file (example for YOLO11s-OBB)

```
python3 export_yolo11_obb.py -w yolo11s-obb.pt --dynamic
```
...
Edit the `config_infer_primary_yolo11_obb.txt` file according to your model (example for YOLO11s-OBB)

```
[property]
...
onnx-file=yolo11s-obb.onnx
...
